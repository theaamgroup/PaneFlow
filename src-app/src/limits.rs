...
/// with `crate::text_sanitize::strip_bidi_zero_width` in the same breath: the
...
pub(crate) fn clamp_untrusted_label(raw: &str) -> String {
    crate::text_sanitize::strip_bidi_zero_width(
        raw.chars().take(MAX_UNTRUSTED_LABEL_CHARS).collect(),
    )
}
