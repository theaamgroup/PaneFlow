...
rename was dropped). Version **0.6.1**. Origin `theaamgroup/paneflow` on
...
./target/debug/paneflow --version          # paneflow 0.6.1
...
│   ├── agent_summary/                 ← Cmd+Shift+I (#576): every agent pane across every workspace with a
│                                         plain-English line generated on-device by Apple's Foundation Models;
│                                         prompt.rs is the pure tail/fence/reply clamp, sidecar.rs drives
│                                         native/agent-summary (Contents/Helpers/paneflow-agent-summary) through
│                                         paneflow-process with a cancel flag; degrades to states-only quietly
...
| `Cmd+Shift+I` | Agent summary (every agent pane, summarised on-device; `agent_summary_enabled` off makes it a no-op) | Global |
...
- **`sidebar_show`** (`crates/paneflow-config/src/schema/config.rs::SidebarShow`): sidebar details controlled by the Customize Sidebar menu (`app/sidebar/customize_menu.rs`) or hot-reloaded config. `branch` defaults to **`true`** and shows each terminal’s current branch beneath its sidebar name; split tabs show a labeled line per terminal. `app/sidebar/branches.rs` probes terminal CWDs off-thread every two seconds, replaces the cache, and triggers the gated PR refresh when it changes. A newly entered CWD has no branch line until its first probe completes. `diffstat` defaults to **`false`** and shows right-aligned checkout counts on workspace rows and explicitly bound tabs; it still uses workspace/bound-tab checkout metadata. `pr` defaults to **`false`** and replaces branch icons with state-colored PR icons from the cached `gh` lookup (requires authenticated `gh` and a GitHub remote). `indent_guide` defaults to **`false`** and draws the workspace-to-tab hairline. Omitting `sidebar_show` uses those defaults. The menu writes the whole object through `config_writer`. Workspace fold state persists separately as `WorkspaceSession.sidebar_collapsed` (written only when folded; session schema stays v2). Issue #489 (upstream `9da2e4be`, persistence half only) adds `TabSession.unread` / `TabSession.pull_request` (`{branch, number, state}`) and `WorkspaceSession.muted` on the same additive terms: `AgentCompletionNotification` is a set of unread surface ids (`record_finished(seen, Option<u64>)`, `is_unread_for(&tab.surface_ids(cx))`, `clear()`), restore replays `record_finished(false, Some(id))` for each unread tab, and a 0.5.0 `session.json` restores quiet and unmuted.
...
- **`agent_summary_enabled`** (issue #576): `None`-is-on master switch for the Agent Summary overlay (`Cmd+Shift+I`, `app/agent_summary/`). The overlay reuses `pane_overview::collect_cards` and keeps only panes with a detected agent or a live session, reads each pane's last 80 rows off the render thread through `scrollback_reader()`, strips controls and caps the tail at `MAX_AGENT_SUMMARY_TAIL_BYTES`, wraps it in the `surface.read` fence (`ipc_handler::wrap_untrusted`, now `pub(crate)`), and runs **one `paneflow-agent-summary` process per pane** through `paneflow_process::run_with_timeout_stdin_cancellable` (20 s deadline, 64 KiB stdout cap, attention-first order; the visual order stays workspace → tab → pane). The sidecar is a single Swift file (`native/agent-summary/main.swift`, built by `scripts/build-agent-summary.sh`, installed by `bundle-macos.sh` as `Contents/Helpers/paneflow-agent-summary` where `sign-macos.sh` already signs it; **not** staged by `build.rs`, so it never counts against `EMBED_SIZE_LIMIT_BYTES`). `FoundationModels.framework` is Swift-only and macOS 26-only: the source compiles as an always-unavailable stub on an older SDK (the release lane's Xcode 16.4 pin), the framework is weak-linked so the helper launches on the macOS 13 floor, and `--probe` answers `{"available":…}` first, so a Mac without Apple Intelligence gets the states-only list with a one-line note, never a dialog. Dismissing the overlay flips the cancel flag (kills the in-flight process) and bumps a generation that discards late replies. For a `cargo run` build drop the helper beside the binary (`scripts/build-agent-summary.sh --out target/debug/paneflow-agent-summary`) or point `PANEFLOW_AGENT_SUMMARY_BIN` at one.
...
- **`#[cfg(unix)]` is not Linux-only.** It appears **177 times** and macOS needs nearly all of it - it is the single highest-risk distinction in this codebase. Do not prune unix-shared code because Linux code sat beside it. `#[cfg(target_os = "macos")]` appears **93** times. Both are live arms and both stay. Counted by the `./scripts/linux-census.sh` negative control (`cfg(unix)` / `cfg(macos)` live sites) on 2026-09-15 at the v0.6.1 cut, the same run recorded in the `docs/fork/STATE.md` verification block - **re-run it before quoting these numbers**. `cfg(unix)` moved 176 -> 177 at that cut (#545 adds one site in the hook code) while `cfg(macos)` held at 93, which is why the figure is corrected here rather than re-attested; the 2026-09-07 and 2026-09-14 counts had both read 176 / 93. They drift with every pass, and the older figures still standing in the fork docs are the earlier counts of the same thing, not a disagreement to resolve: 162 in `docs/fork/2026-08-25-mac-only-fork-design.md`, and 152/77 and 138/71 in dated `STATE.md` entries.
