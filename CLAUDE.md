...
rename was dropped). Version **0.7.0**. Origin `theaamgroup/paneflow` on
...
./target/debug/paneflow --version          # paneflow 0.7.0
...
- **`#[cfg(unix)]` is not Linux-only.** It appears **178 times** and macOS needs nearly all of it - it is the single highest-risk distinction in this codebase. Do not prune unix-shared code because Linux code sat beside it. `#[cfg(target_os = "macos")]` appears **93** times. Both are live arms and both stay. Counted by the `./scripts/linux-census.sh` negative control (`cfg(unix)` / `cfg(macos)` live sites) on 2026-09-18 at the v0.7.0 cut, the same run recorded in the `docs/fork/STATE.md` verification block - **re-run it before quoting these numbers**. `cfg(unix)` moved 177 -> 178 at that cut while `cfg(macos)` held at 93, which is why the figure is corrected here rather than re-attested; the 2026-09-15 v0.6.1 cut read 177 / 93 (176 -> 177 there, #545 adding one site in the hook code), and the 2026-09-07 and 2026-09-14 counts had both read 176 / 93. They drift with every pass, and the older figures still standing in the fork docs are the earlier counts of the same thing, not a disagreement to resolve: 162 in `docs/fork/2026-08-25-mac-only-fork-design.md`, and 152/77 and 138/71 in dated `STATE.md` entries.
