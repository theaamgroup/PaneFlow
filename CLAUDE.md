...
- **`#[cfg(unix)]` is not Linux-only.** It appears **176 times** and macOS needs nearly all of it - it is the single highest-risk distinction in this codebase. Do not prune unix-shared code because Linux code sat beside it. `#[cfg(target_os = "macos")]` appears **93** times. Both are live arms and both stay. Counted by the `./scripts/linux-census.sh` negative control (`cfg(unix)` / `cfg(macos)` live sites) on 2026-09-14 at the v0.6.0 cut, the same run recorded in the `docs/fork/STATE.md` verification block - **re-run it before quoting these numbers**. They happened to land on the same 176 / 93 as the 2026-09-07 count, which is a re-measurement that agreed, not a number carried forward. They drift with every pass, and the older figures still standing in the fork docs are the earlier counts of the same thing, not a disagreement to resolve: 162 in `docs/fork/2026-08-25-mac-only-fork-design.md`, and 152/77 and 138/71 in dated `STATE.md` entries.
