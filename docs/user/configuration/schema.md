...
| `files_tree_placement` | `"rail"` or `"dock"` | `"rail"` | Hosts the Files tree in the existing 300 px right rail, or inside the editor dock at 250 px. Cmd+Alt+F toggles either mode. Dock mode shares a 40 px breadcrumb toolbar with File tabs (including the file picker); Changes, Terminal and Agent setup hide the tree. With no file tab, the chord opens a file picker and the dock. Closing the last file tab closes the tree. The tree hides below 450 px of rendered dock width to keep at least 200 px for the editor, then returns when widened; the dock's 360 px floor and saved width stay unchanged. Open state stays per workspace tab. Sessions can coexist in dock mode; rail mode keeps their mutual exclusion. Both mounts are CLI-only, keep their watcher warm when unmounted in Review or Settings, and open every file, including Markdown, as source. Filter and row context menus work in both modes. Hot-reloads. |
...
| `editor` | object or null | minimap off, scrollbar on | Hot-reloads. What the code editor draws beside the text: `minimap` adds a minimap along the right edge, `scrollbar` keeps the vertical scrollbar. Toggled from the editor's controls menu; the choice applies to every open file. |
