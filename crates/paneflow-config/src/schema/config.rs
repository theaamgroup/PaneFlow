...
pub struct PaneFlowConfig {
    ...
    /// which no system font ships. `None` (or an empty list) adds no
    /// fallback families: Core Text then picks a covering system font for
    /// each glyph the primary lacks. Mirrors Zed's
